...
- The model also needs the fine-tuned document-level embeddings: Download `en-xlm-roberta-large.zip`, `en-roberta-large.zip` and `en-xlnet-large-cased.zip` at [OneDrive](https://1drv.ms/u/s!Am53YNAPSsodg810NxHQcrJpcNIOig?e=FRsJNR) in `fine-tuned models` and unzip them. 
- Change the path of the embeddings (`model:`) in the `config/doc_ner_best.yaml`
#### Evaluation 
To evaluate the accuracy of the model, run:
```bash
...
- The model also needs the fine-tuned document-level embeddings: Download `de-xlm-roberta-large.zip` at [OneDrive](https://1drv.ms/u/s!Am53YNAPSsodg810NxHQcrJpcNIOig?e=FRsJNR) in `fine-tuned models` and unzip them. 
- Change the path of the embeddings (`model:`) in the `config/doc_ner_de_03_best.yaml` or `config/doc_ner_de_06_best.yaml`
...
- The model also needs the fine-tuned document-level embeddings: Download `nl-xlm-roberta-large.zip` at [OneDrive](https://1drv.ms/u/s!Am53YNAPSsodg810NxHQcrJpcNIOig?e=FRsJNR) in `fine-tuned models` and unzip them. 
- Change the path of the embeddings (`model:`) in the `config/doc_ner_nl_best.yaml`.
...
- The model also needs the fine-tuned document-level embeddings: Download `es-xlm-roberta-large.zip` at [OneDrive](https://1drv.ms/u/s!Am53YNAPSsodg810NxHQcrJpcNIOig?e=FRsJNR) in `fine-tuned models` and unzip them. 
- Change the path of the embeddings (`model:`) in the `config/doc_ner_es_best.yaml`.
